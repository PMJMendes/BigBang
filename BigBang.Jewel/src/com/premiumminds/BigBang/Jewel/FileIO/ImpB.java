	...
	extends FileIOBase
{
	...
	public static final UUID FormatID_ImpB = UUID.fromString("EF9F89D4-D3E0-484E-B3CF-A0FB00FFF354");
		...
		throws BigBangJewelException
	{
		...
		try
		{
			createReport(ldb, "Importação Império-Bonança", RDef_Imports, FormatID_ImpB.toString() + "|" + midSession.toString());
		}
		catch (BigBangJewelException e)
		{
			try { ldb.Disconnect(); } catch (SQLException e1) {}
		...
		throws BigBangJewelException
	{
		...
		lstrLineText = parrData[Fields.POLICY].getData() + parrData[Fields.RECEIPT].getData();

		try
		{
			if ( !"Ok".equalsIgnoreCase(parrData[Fields.MARKER].getData()) )
			{
				createDetail(pdb, lstrLineText, plngLine, StatusCodes.Code_6_BadFileLine, null);
				return;
			}
			...
			lstrReceipt = ProcessReceiptNumber(parrData[Fields.RECEIPT].getData(), parrData[Fields.POLICY].getData());
			...
			lidType = ProcessReceiptType(parrData[Fields.TYPE1].getData(), parrData[Fields.TYPE2].getData());
			if ( lidType == null )
			{
			...
			ldblCommission = BuildDecimal(parrData[Fields.COMMISSION1].getData()).
					...
					add(BuildDecimal(parrData[Fields.COMMISSION3].getData()));
	...
	private String ProcessReceiptNumber(String pstrNumber, String pstrPolicy)
	{
		String lstrPartialReceipt;
		String lstrPolicySuffix;

		lstrPartialReceipt = pstrNumber.substring(3, 16);
		lstrPolicySuffix = pstrPolicy.substring(9, 16);

		if ( lstrPartialReceipt.indexOf(lstrPolicySuffix) == 0 )
			return lstrPartialReceipt.substring(7, 13).replaceFirst("^0+(?!$)", "");

		return lstrPartialReceipt.replaceFirst("^0+(?!$)", "");

//		return pstrNumber.substring(0, 12); //JMMM: Não me lembro quando é que este caso acontece...
	}
	...
	private UUID ProcessReceiptType(String pstrType1, String pstrType2)
		throws BigBangJewelException
	{
		String lstrTypeAux;
		IEntity lrefTypes;
		...
		lstrTypeAux = "!" + pstrType1.trim() + ( pstrType2 == null ? "" : " " + pstrType2.trim() );
		lidResult = null;
		...
		if ( (pstrType2 != null) && (lidResult == null)  )
			lidResult = ProcessReceiptType(pstrType1, null);
	...
	{
		return new BigDecimal(pstrNumber.substring(0, pstrNumber.length() - 2) + "." + pstrNumber.substring(pstrNumber.length() - 2));
	}
}
