	...
	extends Operation
{
	...
	public AssessmentData mobjData;

	public CreateAssessment(UUID pidProcess)
	{
		super(pidProcess);
	}
	...
	protected UUID OpID()
	{
		return Constants.OPID_SubCasualty_CreateAssessment;
	}
		...
		throws JewelPetriException
	{
		...
		try
		{
			...
			lobjAux = Assessment.GetInstance(Engine.getCurrentNameSpace(), (UUID)null);
			mobjData.ToObject(lobjAux);
			lobjAux.SaveToDb(pdb);

			lobjScript = PNScript.GetInstance(Engine.getCurrentNameSpace(), Constants.ProcID_Assessment);
		...
		throws BigBangJewelException
	{
		...
		try
		{
	        lstrFilter = GetProcess().GetData().getLabel() + ".PA%";
			lrefCasualties = Entity.GetInstance(Engine.FindEntity(Engine.getCurrentNameSpace(), Constants.ObjID_Assessment)); 
			ldb = new MasterDB();
		}
		catch (Throwable e)
		{
