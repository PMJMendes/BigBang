	...
	extends UndoableOperation
{
	...
	protected UUID OpID()
	{
		return Constants.OPID_SubPolicy_ManageData;
	}
	...
	public String LongDesc(String pstrLineBreak)
	{
		...
		if ( mobjData != null )
		{
			...
			{
				lstrResult.append("Novos dados da apólice adesão:").append(pstrLineBreak);
		...
		try
		{
			if ( mobjData != null )
			{
				...
				if ( mobjData.mbModified )
				{
					...
					mobjData.mobjPrevValues = new SubPolicyData();
				...
				if ( mobjData.marrCoverages != null )
				{
					for ( i = 0; i < mobjData.marrCoverages.length; i++ )
					{
						...
						else
						{
							lobjCoverage = SubPolicyCoverage.GetInstance(Engine.getCurrentNameSpace(),
									mobjData.marrCoverages[i].mid);
							mobjData.marrCoverages[i].mobjPrevValues = new SubPolicyCoverageData();
				...
				if ( mobjData.marrObjects != null )
				{
					for ( i = 0; i < mobjData.marrObjects.length; i++ )
					{
						...
						{
							lobjObject = SubPolicyObject.GetInstance(Engine.getCurrentNameSpace(),
									mobjData.marrObjects[i].mid);
							mobjData.marrObjects[i].mobjPrevValues = new SubPolicyObjectData();
					...
					for ( i = 0; i < mobjData.marrValues.length; i++ )
					{
						if ( mobjData.marrValues[i].mbDeleted )
						{
							if ( mobjData.marrValues[i].mid == null )
								continue;
							lobjValue = SubPolicyValue.GetInstance(Engine.getCurrentNameSpace(),
						...
						else
						{
							lobjValue = SubPolicyValue.GetInstance(Engine.getCurrentNameSpace(),
									mobjData.marrValues[i].mid);
							mobjData.marrValues[i].mobjPrevValues = new SubPolicyValueData();
			...
			if ( mobjData != null )
			{
				...
				if ( mobjData.marrCoverages != null )
				{
					for ( i = 0; i < mobjData.marrCoverages.length; i++ )
					{
						if ( mobjData.marrCoverages[i].mbNew )
						{
							lobjCoverage = SubPolicyCoverage.GetInstance(Engine.getCurrentNameSpace(), mobjData.marrCoverages[i].mid);
							lobjCoverage.getDefinition().Delete(pdb, lobjCoverage.getKey());
						}
						else
						{
				...
				if ( mobjData.marrObjects != null )
				{
					for ( i = 0; i < mobjData.marrObjects.length; i++ )
					{
						...
						{
							lobjObject = SubPolicyObject.GetInstance(Engine.getCurrentNameSpace(), mobjData.marrObjects[i].mid);
				...
				if ( mobjData.marrValues != null )
				{
					for ( i = 0; i < mobjData.marrValues.length; i++ )
					{
						...
						{
							lobjValue = SubPolicyValue.GetInstance(Engine.getCurrentNameSpace(), mobjData.marrValues[i].mid);
							lobjValue.getDefinition().Delete(pdb, lobjValue.getKey());
						}
						else
						{
