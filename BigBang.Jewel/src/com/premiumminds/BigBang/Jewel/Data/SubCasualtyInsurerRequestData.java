...
public class SubCasualtyInsurerRequestData implements DataBridge {
	...
	public void FromObject(ObjectBase source) {
		...
		conforms = (Boolean) source
				.getAt(SubCasualtyInsurerRequest.I.CONFORMITY);
		requestDate = (Timestamp) source
				.getAt(SubCasualtyInsurerRequest.I.REQUESTDATE);
		acceptanceDate = (Timestamp) source
				.getAt(SubCasualtyInsurerRequest.I.ACCEPTANCEDATE);
		resendDate = (Timestamp) source
				.getAt(SubCasualtyInsurerRequest.I.RESENDDATE);
		clarificationDate = (Timestamp) source
	...
	public void ToObject(ObjectBase dest) throws BigBangJewelException {
		try {
			...
			dest.setAt(SubCasualtyInsurerRequest.I.ACCEPTANCEDATE,
					acceptanceDate);
	...
	public void Describe(StringBuilder pstrBuilder, String pstrLineBreak) {
		...
		if (typeId != null) {
			try {
				...
				pstrBuilder
						.append("(Erro a obter o tipo de pedido de segurador.)");
			}
		} else {
		...
		if (conforms) {
			...
			if (clarificationDate != null) {
				pstrBuilder.append(clarificationDate.toString()
						.substring(0, 19));
			} else {
				pstrBuilder.append("Não indicada.");
			}
