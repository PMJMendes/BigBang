...
public class MassParticipateToInsurerView extends View implements MassParticipateToInsurerViewPresenter.Display{
	...
	protected static class SelectedExpensesList extends SelectedProcessesList<HealthExpenseStub>{

		@Override
		public ListEntry<HealthExpenseStub> addEntry(HealthExpenseStub value) {
			ListEntry<HealthExpenseStub> entry = new ListEntry<HealthExpenseStub>(value);
	...
	protected static class CheckableExpensesSearchPanel extends CheckableSearchPanel<HealthExpenseStub>{
		...
		@Override
		public ListEntry<HealthExpenseStub> addEntry(HealthExpenseStub value) {
			ListEntry<HealthExpenseStub> entry = new ExpenseSearchPanel.Entry(value);
			add(0, entry);
			return entry;
		}
	...
	@Override
	public void addExpenseToParticipate(HealthExpenseStub stub) {
	...
	public void removeExpenseToParticipate(String id) {
		for(ValueSelectable<HealthExpenseStub> entry : selectedExpenses){
	...
	@Override
	public HasEditableValue<HealthExpense> getExpenseForm() {
		return expenseForm;
	}

	@Override
	public HasValueSelectables<HealthExpenseStub> getMainList() {
		return searchPanel;
	}

	@Override
	public HasValueSelectables<HealthExpenseStub> getSelectedList() {
	...
	public void markAllForCheck() {
		if(searchPanel.getWorkspaceId() != null){
			SearchDataBroker<HealthExpenseStub> broker = this.searchPanel.getSearchBroker();
			broker.getResults(searchPanel.getWorkspaceId(), 0, -1, new ResponseHandler<Search<HealthExpenseStub>>() {

				@Override
				public void onResponse(Search<HealthExpenseStub> response) {
					Collection <HealthExpenseStub> results = response.getResults();
					for(HealthExpenseStub result : results){
