...
import bigBang.module.expenseModule.client.userInterface.presenter.ProofReceptionViewPresenter.Action;
...
public class ProofReceptionView extends View implements ProofReceptionViewPresenter.Display {
	...
	protected ExpenseImagePanel imagePanel;
	protected ActionInvokedEventHandler<Action> actionHandler;
	protected CheckableExpensesSearchPanel searchPanel;
	protected SelectedExpensesList selectedExpenses;
	protected Button receiveProof, clearButton;

	public ProofReceptionView(){
		SplitLayoutPanel wrapper = new SplitLayoutPanel();
		initWidget(wrapper);
		wrapper.setSize("100%", "100%");

		receiveProof = new Button("Confirmar Recepção");

		HorizontalPanel confirmClearWrapper = new HorizontalPanel();
		confirmClearWrapper.setVerticalAlignment(HasVerticalAlignment.ALIGN_MIDDLE);
		confirmClearWrapper.setHeight("100%");
		confirmClearWrapper.add(receiveProof);
		receiveProof.addClickHandler(new ClickHandler() {

			@Override
			public void onClick(ClickEvent event) {
				actionHandler.onActionInvoked(new ActionInvokedEvent<Action>(Action.RECEIVE_PROOF));
			}
		});
		...
		participateToInsurerForm.addSection("Receber Comprovativo");

		VerticalPanel selectedListWrapper = new VerticalPanel();
		selectedListWrapper.add(new ListHeader("Receber Comprovativo"));
		selectedListWrapper.setSize("100%", "100%");
		participateToInsurerForm.addWidget(confirmClearWrapper);
		...
		confirmClearWrapper.add(clearButton);
		confirmClearWrapper.setSpacing(5);
		...
		searchPanel.setOperationId(BigBangConstants.OperationIds.ExpenseProcess.RECEIVE_PROOF);
		...
		wrapper.addEast(searchPanelWrapper, 400);
		
		
		
		imagePanel = new ExpenseImagePanel(true, false);
		imagePanel.setHeaderText("Imagem do Comprovativo");
		imagePanel.setSize("100%", "100%");
		wrapper.add(imagePanel);
	}
	
	@Override
	public void addExpenseToSelection(ExpenseStub stub) {
	...
	@Override
	public void removeExpenseFromSelection(String id) {
	...
	@Override
	public void removeAllExpensesFromSelection() {
	...
	@Override
	public void allowConfirm(boolean b) {
		receiveProof.setEnabled(b);
	...
	public NavigationPanel getNavigationPanel() {
		return imagePanel.getNavigationPanel();
	}
