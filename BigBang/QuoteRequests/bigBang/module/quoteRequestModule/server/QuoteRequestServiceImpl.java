...
import com.premiumminds.BigBang.Jewel.Operations.QuoteRequest.CreateMgrXFer;
...
//					throw new BigBangException("Inesperado: Valor variável marcado como invariante.");
		...
		throws SessionExpiredException, BigBangException
	{
		...
		try
		{
			...
			lobjCMX = new CreateMgrXFer(lobjRequest.GetProcessID());
		...
		transfer.objectStubs = new SearchResult[] {TransferManagerServiceImpl.sBuildStub(Constants.ObjID_QuoteRequest,
				UUID.fromString(transfer.dataObjectIds[0]))};
		transfer.managedProcessIds = new String[] {lobjRequest.GetProcessID().toString()};
		...
		throws SessionExpiredException, BigBangException
	{
		...
		try
		{
			...
			else
			{
				...
				lobjItem.setAt(0, "Transferência de Gestor de Consulta de Mercado");
		...
		for ( i = 0; i < larrProcessIDs.length; i++ )
		{
			transfer.objectStubs[i] = TransferManagerServiceImpl.sBuildStub(Constants.ObjID_QuoteRequest,
					UUID.fromString(transfer.dataObjectIds[i]));
