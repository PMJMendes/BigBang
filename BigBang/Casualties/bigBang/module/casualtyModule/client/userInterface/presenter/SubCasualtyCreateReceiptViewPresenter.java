...
public class SubCasualtyCreateReceiptViewPresenter implements ViewPresenter {
	...
	public static interface Display {
		...
		HasValue<SubCasualty> getParentForm();
	...
	public SubCasualtyCreateReceiptViewPresenter(Display view){
	...
	public void setParameters(HasParameters parameterHolder) {
		String ownerId = parameterHolder.getParameter("subcasualtyid");
	...
	protected void bind(){
		...
		view.registerActionHandler(new ActionInvokedEventHandler<SubCasualtyCreateReceiptViewPresenter.Action>() {
	...
	protected void showCreateReceipt(String ownerId){
		...
		broker.getSubCasualty(ownerId, new ResponseHandler<SubCasualty>() {
			...
			public void onResponse(SubCasualty response) {
				...
				receipt.ownerTypeId = BigBangConstants.EntityIds.SUB_CASUALTY;
				receipt.ownerNumber = response.number;
				receipt.categoryName = response.categoryName;
				receipt.managerId = response.managerId;
				receipt.mediatorId = response.inheritMasterMediatorId;
				receipt.lineName = response.lineName;
				receipt.subLineName = response.subLineName;
				receipt.clientName = response.inheritMasterClientName;
				receipt.clientNumber = response.inheritMasterClientNumber;
				receipt.clientId = response.inheritMasterClientId;
				receipt.inheritEndDate = null;
