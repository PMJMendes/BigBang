...
public class TotalLossFileViewPresenter implements ViewPresenter{
	...
	public interface Display{
		HasEditableValue<TotalLossFile> getForm();
	...
	private String totalLossFileId;
	private String subCasualtyId;
	protected TotalLossFile totalLossFile;
	
	public TotalLossFileViewPresenter(Display view) {
	...
	protected void showConversation(String id) {
		...
		item.pushIntoStackParameter("display", "totallossfileconversation");
	...
	protected void onNavigateBack() {
		...
		item.removeParameter("totallossfileid");
		NavigationHistoryManager.getInstance().go(item);					
	}
	...
	protected void onSendMessage() {
		NavigationHistoryItem item = NavigationHistoryManager.getInstance().getCurrentState();
		item.pushIntoStackParameter("display", "totallossfilesendmessage");
		NavigationHistoryManager.getInstance().go(item);				
		
	}

	protected void onSave() {
		if(totalLossFileId.equalsIgnoreCase("new")){
			subCasualtyBroker.createTotalLossFile(view.getForm().getInfo(), new ResponseHandler<TotalLossFile>() {

				@Override
				public void onResponse(TotalLossFile response) {
					EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Perda Total criada com sucesso."), TYPE.TRAY_NOTIFICATION));
					totalLossFile = response;
					totalLossFileId = response.id;
					NavigationHistoryItem item = NavigationHistoryManager.getInstance().getCurrentState();
					item.setParameter("totallossfileid", totalLossFileId);
					NavigationHistoryManager.getInstance().go(item);
				}

				@Override
				public void onError(Collection<ResponseError> errors) {
					EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Não foi possível criar a Perda Total."), TYPE.ALERT_NOTIFICATION));
				}
			});
		}
		else{
			broker.editTotalLossFile(view.getForm().getInfo(), new ResponseHandler<TotalLossFile>() {
	...
	protected void getTotalLoss() {
		if(totalLossFileId.equalsIgnoreCase("new")){
			view.lockToolbar();
			view.setSaveMode(true);
			view.allowEdit(true);
			view.getForm().setReadOnly(false);
			TotalLossFile file = new TotalLossFile();
			file.subCasualtyId = subCasualtyId;
			view.getForm().setValue(file);
		}

		else{
			broker.getTotalLossFile(totalLossFileId, new ResponseHandler<TotalLossFile>() {
	...
	protected void setPermissions() {
		view.lockToolbar();
		view.allowClose(PermissionChecker.hasPermission(totalLossFile, BigBangConstants.OperationIds.TotalLossFileProcess.CLOSE));
		view.allowReceiveMessage(PermissionChecker.hasPermission(totalLossFile, BigBangConstants.OperationIds.TotalLossFileProcess.CONVERSATION));
		view.allowSendMessage(PermissionChecker.hasPermission(totalLossFile, BigBangConstants.OperationIds.TotalLossFileProcess.CONVERSATION));
		view.allowEdit(PermissionChecker.hasPermission(totalLossFile, BigBangConstants.OperationIds.TotalLossFileProcess.EDIT));		
	}

	protected void onReceiveMessage() {
		NavigationHistoryItem item = NavigationHistoryManager.getInstance().getCurrentState();
		item.pushIntoStackParameter("display", "totallossfilereceivemessage");
		item.setParameter("ownerid", item.getParameter("totallossfileid"));
		NavigationHistoryManager.getInstance().go(item);								
	}

	protected void onClose() {
		broker.closeProcess(totalLossFileId, new ResponseHandler<TotalLossFile>() {
	...
	public void setParameters(HasParameters parameterHolder) {
		totalLossFileId = parameterHolder.getParameter("totallossfileid");
