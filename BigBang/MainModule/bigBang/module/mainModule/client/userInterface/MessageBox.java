...
public abstract class MessageBox extends WindowPanel {
	...
	private static void alert(MessageBoxType type, String caption, String message) {
		...
		alert.setWidget(new ScrollPanel(new HTML(message)));
		alert.showModal();

		if(alert.getOffsetHeight() > maxHeight){
			alert.widget.setHeight(maxHeight+"px");
			alert.setHeight(maxHeight + "px");
		}
	...
	private static void alert(MessageBoxType type, String caption, String message, final String details) {
		final MessageBox alert = new MessageBox(type, caption) {
			@Override
			public void onClose(boolean result) {
				hide();
		...
		final Button buttonDetails = new Button("Detalhes", new ClickHandler() {
			
			@Override
			public void onClick(ClickEvent event) {
				alert.hide();
				alert("Detalhes do Erro", details);
			}
		});
		alert.getButtonPanel().add(buttonDetails);
		alert.getButtonPanel().add(buttonOK);
	...
	public static void detailedError(String caption, String message, String details) {
		alert(MessageBoxType.ERROR, caption, message, details);
	}
