...
public class SubPolicyObjectServiceImpl
	extends SearchServiceBase
	implements SubPolicyObjectService
{
		...
		throws SessionExpiredException, BigBangException
	{
		...
		try
		{
			lobjObject = SubPolicyObject.GetInstance(Engine.getCurrentNameSpace(), lidObject);
			...
			lobjPolicy = SubPolicy.GetInstance(Engine.getCurrentNameSpace(), (UUID)lobjObject.getAt(1));
			...
			larrVariable = new SubPolicyValue[larrExercises.length][];
	...
	{
		return Constants.ObjID_SubPolicyObject;
	}

	protected String[] getColumns()
	{
		return new String[] {"[:Name]", "[:Sub Policy]", "[:Type]", "[:Type:Type]", "[:Address 1]", "[:Address 2]", "[:Zip Code]",
		...
		throws BigBangException
	{
		...
		{
			pstrBuffer.append(" AND [:Sub Policy] = '").append(lParam.policyId).append("'");
		}
