	...
	private static class SubPolicyScratchPad
	{
			...
			throws CorruptedPadException
		{
			...
			for ( i = 0; i < marrValues.size(); i++ )
			{
				...
				lobjHeader.type = InsurancePolicyServiceImpl.sGetFieldTypeByID(marrValues.get(i).mrefField.midType);
			...
			for ( i = 0; i < marrCoverages.size(); i++ )
			{
				...
				for ( j = 0; j < marrCoverages.get(i).marrFields.length; j++ )
				{
					...
					lobjColumn.type = InsurancePolicyServiceImpl.sGetFieldTypeByID(marrCoverages.get(i).marrFields[j].midType);
			...
			for ( i = 0; i < marrValues.size(); i++ )
			{
				...
				lobjExtraField.type = InsurancePolicyServiceImpl.sGetFieldTypeByID(marrValues.get(i).mrefField.midType);
		...
		for ( i = 0; i < larrLocalCoverages.length; i++ )
		{
			...
			for ( j = 0; j < larrTaxes.length ; j++ )
			{
				...
				if ( !larrOutColumns.containsKey(larrTaxes[j].GetColumnOrder()) )
				{
					...
					lobjColumnHeader.type = InsurancePolicyServiceImpl.sGetFieldTypeByID((UUID)larrTaxes[j].getAt(2));
		...
		for ( i = 0; i < larrCoverages.length; i++ )
		{
			...
			for ( j = 0; j < larrTaxes.length; j++ )
			{
				...
				if ( larrCoverages[i].IsHeader() )
				{
					...
					lobjHeader.type = InsurancePolicyServiceImpl.sGetFieldTypeByID((UUID)larrTaxes[j].getAt(2));
				...
				else
				{
					...
					lobjExtra.type = InsurancePolicyServiceImpl.sGetFieldTypeByID((UUID)larrTaxes[j].getAt(2));
