...
public class MassReceiptGenerationViewPresenter implements ViewPresenter {

	public enum Action{
		SELECT_ALL, RECEIPT_GENERATION, CLEAR
	...
	public MassReceiptGenerationViewPresenter(Display view){
		...
		view.registerActionHandler(new ActionInvokedEventHandler<Action>() {
			...
			public void onActionInvoked(ActionInvokedEvent<Action> action) {
				...
				case RECEIPT_GENERATION:
					generateReceipts(view.getSelectedList().getAll());
					break;
				case SELECT_ALL:
	...
	protected void generateReceipts(Collection<ValueSelectable<ReceiptStub>> all) {
		...
		broker.massGenerateReceipt(receiptIds, new ResponseHandler<Void>() {
			
			@Override
			public void onResponse(Void response) {
				onGenerateReceiptSuccess();
			}
			
			@Override
			public void onError(Collection<ResponseError> errors) {
				onGenerateReceiptFail();
			}
		});
		
	}

	protected void onGenerateReceiptFail() {
		EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Não foi possível emitir os recibos"), TYPE.ALERT_NOTIFICATION));
	}

	protected void onGenerateReceiptSuccess() {
		EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Recibos emitidos com sucesso"), TYPE.TRAY_NOTIFICATION));
