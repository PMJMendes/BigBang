...
public class MassCreateSecondPaymentNoticeViewPresenter implements ViewPresenter{

	private Display view;
	private boolean bound = false;
	protected ReceiptDataBroker broker;

	public MassCreateSecondPaymentNoticeViewPresenter(Display view){
	...
	public enum Action{
		SELECT_ALL, CREATE_SECOND_PAYMENT_NOTICES, CLEAR
		...
		view.registerActionHandler(new ActionInvokedEventHandler<Action>() {
			...
			public void onActionInvoked(ActionInvokedEvent<Action> action) {
				...
				case CLEAR:
					...
					break;
				case CREATE_SECOND_PAYMENT_NOTICES:
	...
	public void createPaymentNotices(Collection<ValueSelectable<ReceiptStub>> collection){
		...
		broker.massCreateSecondPaymentNotice(receiptIds, new ResponseHandler<Void>() {

			@Override
			public void onResponse(Void response) {
				EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Segundos avisos de cobrança criados com êxito"), TYPE.TRAY_NOTIFICATION));
				NavigationHistoryManager.getInstance().reload();
			}

			@Override
			public void onError(Collection<ResponseError> errors) {
				EventBus.getInstance().fireEvent(new NewNotificationEvent(new Notification("", "Ocorreu um erro ao enviar segundos avisos de cobrança"), TYPE.ALERT_NOTIFICATION));				
			}
		});
