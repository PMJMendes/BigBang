...
import bigBang.module.receiptModule.client.userInterface.presenter.MassReceiptGenerationViewPresenter.Action;
...
public class MassReceiptGenerationView extends View implements MassReceiptGenerationViewPresenter.Display{
	...
	protected static enum Filters {
		...
		CATEGORY,
		LINE,
		SUB_LINE, AGENCY, MANAGER, MEDIATOR, IS_INTERNAL
	}
	...
	protected static class CheckableReceiptsSearchPanel extends CheckableSearchPanel<ReceiptStub>{
		...
		public CheckableReceiptsSearchPanel(){
			...
			this.setOperationId(BigBangConstants.OperationIds.ReceiptProcess.RECEIPT_GENERATION);
	...
	protected ActionInvokedEventHandler<MassReceiptGenerationViewPresenter.Action> actionHandler;
	...
	public MassReceiptGenerationView() {
		...
		Button selectAllButton = new Button("Seleccionar Todos", new ClickHandler() {
			...
			@Override
			public void onClick(ClickEvent event) {
				actionHandler.onActionInvoked(new ActionInvokedEvent<MassReceiptGenerationViewPresenter.Action>(MassReceiptGenerationViewPresenter.Action.SELECT_ALL));
			}
		});
		...
		createPNotice.addClickHandler(new ClickHandler() {
			...
			@Override
			public void onClick(ClickEvent event) {
				actionHandler.onActionInvoked(new ActionInvokedEvent<Action>(Action.RECEIPT_GENERATION));
			}
		});
	...
	public void registerActionHandler(
			ActionInvokedEventHandler<MassReceiptGenerationViewPresenter.Action> actionInvokedEventHandler) {
