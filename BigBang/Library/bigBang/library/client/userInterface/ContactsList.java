...
public class ContactsList extends FilterableList<Contact> implements ContactsBrokerClient {
	...
	public ContactsList(){
		...
		this.broker = (ContactsBroker) DataBrokerManager.staticGetBroker(BigBangConstants.EntityIds.CONTACT);
	...
	public void setSelected(ListEntry<Contact> entry, boolean fireEvents){
		if(entry == null){
			clearSelection();
		}
		else {
			for(ListEntry<Contact> con: this){
				if (con.equals(entry)){
