...
inner join credegs..empresa.cliente c on c.cliente=s.cliente
inner join credegs..empresa.ramos h on h.ramo=r.ramo
left outer join credegs..empresa.comissagentes g on g.fkagente=isnull(s.mediapol, c.mediacli) and g.fkramo=r.ramo
...
update credite_egs.tblBBReceipts set FKProcess=p.PK
from credite_egs.tblBBReceipts r inner join credite_egs.tblPNProcesses p on p.FKData=r.PK;
...
inner join credegs..empresa.historico h on h.cliente=x.cliente and h.apolice=x.apolice and h.recibo=x.recibo and h.comseg=x.comseg and h.ramo=x.ramo
inner join credite_egs.tblPNProcesses p on p.FKData=r.PK
