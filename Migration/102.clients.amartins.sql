...
FixedPercent, CalcClass, BHasRetention)
select CAST(CAST(NEWID() AS BINARY(10)) + CAST(GETDATE() AS BINARY(6)) AS UNIQUEIDENTIFIER) PK,
a.NOME MediatorName, a.NUMCONTR FiscalNulber, 'C7236BA7-73AD-40ED-B6DC-9EFC013691C8' FKProfile, a.MORADA Address1, c.PK FKZipCode,
a.AGENTE MigrationID, NULL, NULL, 0
from amartins..empresa.agente a
left outer join bigbang.tblPostalCodes c on c.PostalCode=CAST(a.CODPOSTAL AS VARCHAR(20)) COLLATE DATABASE_DEFAULT
...
(select distinct codpostal PostalCode, rtrim(ltrim(upper(locpostal))) PostalCity
from amartins..empresa.cliente s left outer join bigbang.tblpostalcodes c on ltrim(s.codpostal)=c.postalcode COLLATE DATABASE_DEFAULT
...
m.PK FKMediator,
CASE c.ClienteVIP WHEN 1 THEN '63114D11-6087-4EFE-9A7E-9EE600BE52DA' ELSE '51ED12A4-95A9-44B0-928D-A01500DC83EB' END FKProfile,
...
insert into amartins.tblBBInsurerReceipts (PK, RecNumber, FKCompany, Value, Tax, RecDate)
select CAST(CAST(NEWID() AS BINARY(10)) + CAST(GETDATE() AS BINARY(6)) AS UNIQUEIDENTIFIER) PK,
...
from amartins..empresa.facturas f
inner join amartins.tblCompanies c on c.MigrationID=f.fkcomseg;
