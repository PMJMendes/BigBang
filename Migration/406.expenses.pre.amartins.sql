...
inner join amartins.tblInsuredObjects o on o.FKPolicy=p.PK and o.ObjName=CAST(e.NumObjecto AS NVARCHAR(50))
...
from amartins.tblBBExpenses e
inner join amartins..empresa.saudedespesas d on d.IDDesp=e.ENumber
inner join amartins..empresa.apolice a on a.cliente=d.cliente and a.apolice=d.apolice and a.ramo=d.ramo and a.comseg=d.comseg
inner join amartins.tblBBPolicies p on p.MigrationID=a.MigrationID
...
inner join amartins..empresa.apolice a on a.cliente=d.cliente and a.apolice=d.apolice and a.ramo=d.ramo and a.comseg=d.comseg
inner join amartins.tblBBSubPolicies s on s.MigrationID=a.MigrationID
inner join amartins.tblPNProcesses k on k.PK=s.FKProcess;

update amartins.tblBBExpenses set FKProcess=p.PK
from amartins.tblBBExpenses e inner join amartins.tblPNProcesses p on p.FKData=e.PK;
...
from amartins.tblBBExpenses e
inner join amartins.tblPNProcesses p on p.FKData=e.PK
inner join amartins..empresa.saudedespesas d on d.IDDesp=e.ENumber
inner join amartins..empresa.saudehistorico h on h.FKDesp=d.IDDesp
