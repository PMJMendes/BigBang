...
import bigBang.module.expenseModule.client.ExpenseSection;

public class ExpenseSectionViewPresenter implements SectionViewPresenter {
	...
	public ExpenseSectionViewPresenter(EventBus eventBus, Service service, View view) {
	...
	public void setSection(MenuSection s) {
		this.expenseSection = (ExpenseSection) s;
		OperationViewPresenter[] operationPresenters = this.expenseSection.getOperationPresenters();
		for(int i = 0; i < operationPresenters.length; i++)
			registerOperation(operationPresenters[i]);
	}
	
	public MenuSection getSection() {
		return this.expenseSection;
	}
